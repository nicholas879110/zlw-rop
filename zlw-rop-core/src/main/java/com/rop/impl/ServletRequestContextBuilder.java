...
public class ServletRequestContextBuilder implements RequestContextBuilder {
    ...
    public SimpleRopRequestContext buildBySysParams(RopContext ropContext,
        ...
        //add 2017-05-31增加终端设备信息 fukui
        requestContext.setDevice(servletRequest.getParameter(SystemParameterNames.getDevice()));
        
        if( logger.isDebugEnabled() ){
        	logger.debug("Request Info : " + requestContext.toString());
        }
    ...
    private AbstractRopRequest buildRopRequestFromBindingResult(RopRequestContext ropRequestContext, BindingResult bindingResult) {
        ...
        if (ropRequest instanceof AbstractRopRequest) {
            ...
            if( abstractRopRequest!=null){
            	Map<String,String> allParams = abstractRopRequest.getRopRequestContext().getAllParams();
            	String userParams = allParams.get("params") ;
            	logger.debug(" userParams : "+userParams);
            	if( allParams !=null && userParams !=null){
            		try {
            			ropRequest = ropRequest.getClass().newInstance();
            			ropRequest.setRopRequestContext(ropRequestContext);
            			try {
            				ropRequest = mapper.readValue(userParams, ropRequest.getClass());
						} catch (Exception e) {
							logger.error("userParams -> ropRequest error : " , e);
						}
            			//使用jackson直接将json转换成目标java对象
            			//	ReflectionBeanHelper.setFieldValue(ropRequest, userParams);
