...
public class AnnotationServletServiceRouter implements ServiceRouter {
    ...
    private class ServiceRunnable implements Runnable {
        ...
        public void run() {
            ...
            long serviceThreadStartTime = System.currentTimeMillis();
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("<服务统计>调用服务方法:" + ropRequestContext.getMethod() + "(" + ropRequestContext.getVersion() + ")，耗时：" + (System.currentTimeMillis() - serviceThreadStartTime) + "ms");
                    logger.debug("输出响应：" + MessageMarshallerUtils.getMessage(ropRequestContext.getRopResponse(), ropRequestContext.getMessageFormat()));
                }
            } catch (Throwable e) {
