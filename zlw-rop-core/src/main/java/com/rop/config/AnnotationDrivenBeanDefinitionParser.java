...
import com.rop.security.FileMethodAuthorizationManager;
...
public class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        //会话管理器
        RuntimeBeanReference methodAuthorizationManager = getMethodAuthorizationManager(element, source, parserContext);
        serviceRouterDef.getPropertyValues().add("methodAuthorizationManager", methodAuthorizationManager);
    ...
    private RuntimeBeanReference getMethodAuthorizationManager(Element element, Object source, ParserContext parserContext) {
        RuntimeBeanReference methodAuthorizationManagerRef;
        if (element.hasAttribute("method-authorization-manager")) {
            methodAuthorizationManagerRef = new RuntimeBeanReference(element.getAttribute("method-authorization-manager"));
        } else {
            RootBeanDefinition methodAuthorizationManager = new RootBeanDefinition(FileMethodAuthorizationManager.class);
            methodAuthorizationManager.setSource(source);
            methodAuthorizationManager.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
            String sessionManagerName = parserContext.getReaderContext().registerWithGeneratedName(methodAuthorizationManager);
            parserContext.registerComponent(new BeanComponentDefinition(methodAuthorizationManager, sessionManagerName));
            methodAuthorizationManagerRef = new RuntimeBeanReference(sessionManagerName);
        }
        return methodAuthorizationManagerRef;
    }
