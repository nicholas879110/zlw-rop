...
public class DefaultSecurityManager implements SecurityManager {
    ...
    protected MethodAuthorizationManager methodAuthorizationManager = new FileMethodAuthorizationManager();
    ...
    public MainError validateSystemParameters(RopRequestContext context) {
        ...
        //9.检查 方法访问权限
        if (!methodAuthorizationManager.isAuthorized(context.getAppKey(), context.getMethod(), context.getVersion())) {
            return MainErrors.getError(MainErrorType.METHOD_ACCESS_UNAUTHORIZED, context.getLocale(),
                    context.getMethod(), context.getVersion(), context.getAppKey());
        }
        return null;
    }
    ...
    public void setMethodAuthorizationManager(MethodAuthorizationManager methodAuthorizationManager) {
        this.methodAuthorizationManager = methodAuthorizationManager;
    }
        ...
        if (context.isSignEnable()) {
            if (!context.getServiceMethodDefinition().isIgnoreSign()) {
                if (context.getSign() == null) {
                    ...
                    String params = needSignParams.get("params");
                    String signValue = RopUtils.sign(params, signSecret);

                    //解密参数
                    String paramsKey = context.getAllParams().get("paramsKey");
                    String paramSecret= getAppSecretManager().getSecret(paramsKey);
                    //params = AES.Decrypt(params, signSecret);
                    params = AES.Decrypt(params, paramSecret);
                    context.getAllParams().put("params", params);

                    //String signValue = RopUtils.sign(needSignParams, signSecret);
                    if (!signValue.equalsIgnoreCase(context.getSign())) {
